...
def main() -> None:
    ...
    with stage("compute", rep):
        ...
        # PCA is deterministic given input, but neighbors takes a
        # random_state — seed per replicate so we get genuine across-rep
        # variance independent of contention noise.
        sc.tl.pca(adata, n_comps=args.n_comp, random_state=rep * 1000)
        sc.pp.neighbors(adata, n_neighbors=15, random_state=rep * 1000)
        # UMAP commented out: it dominates compute wall and obscures the
        # read-contention signal we're after. Re-enable for step 2 (worker
        # scaling) when compute-time variance is the metric of interest.
